#!/usr/bin/env python3
"""
Round 4: Four style references + identity photo.

Style refs: Fernando Alonso, Toto Wolff, Fred Vasseur, Charles Leclerc
Identity: George Russell real photo

Uses v10 hybrid approach (JSON routing header + natural language).

Usage:
    source .env
    python test_json_4refs.py
"""
...
def main():
    ...
    print("=" * 60)
    print("ROUND 4: Four Style References + Identity")
    print("=" * 60)

    # Build image parts: 4 style refs first, then identity last
    style_parts = []
    for path in STYLE_IMAGES:
        print(f"  Loading style ref: {Path(path).name}")
        style_parts.append(make_image_part(path))

    print(f"  Loading identity:  George Russell")
    id_part = make_image_part(IDENTITY_IMAGE)

    # v10-style hybrid JSON + natural language, adapted for 4 refs
    schema = {
        "image_sources": {
            ...
            "IMAGE_5": {"role": "identity", "character": "George Russell real photo"}
        },
        "style_lock": {
        ...
        "text_override": {
            "clothing": "black Mercedes-AMG Petronas F1 team suit with Petronas teal accents"
        },
    ...
    print(f"\n  Prompt: {len(prompt)} chars, {len(style_parts)} style refs + 1 identity")
    print()

    # Parts order: style1, style2, style3, style4, identity, text
    all_parts = style_parts + [id_part, {"text": prompt}]

    result = call_gemini(api_key, all_parts, "v12_4refs")
    if result:
        save(result, "george_nanobana_v12.png")

    print("\n" + "=" * 60)
    print("DONE — Check test-output/george_nanobana_v12.png")
    print("=" * 60)
