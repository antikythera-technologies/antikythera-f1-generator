...
"""
        ...
        async with OviSpaceManager(quality=settings.OVI_QUALITY) as ovi_manager:
            for scene in scenes:
                ...
                try:
                    ...
                    # Generate scene image with ComfyUI (Flux + LoRA + PuLID)
    ...
    async def _get_scene_image(self, db: AsyncSession, scene: Scene) -> str:
        """
        Generate scene image using ComfyUI (Flux + LoRA + PuLID).

        Character consistency is maintained through:
        1. ANTKF1STYLE LoRA for consistent satirical caricature style
        2. PuLID face conditioning from reference headshot
        3. Rich personality traits from character-system JSON files
        """
        ...
        if scene.character_id:
            ...
            if character:
                ...
                # Load rich traits from personality JSON (same as character API)
    ...
    async def _stitch_video(self, db: AsyncSession) -> str:
        ...
        for scene in scenes:
            if scene.video_clip_path:
                ...
                # Parse bucket and object name from stored path (format: "bucket/object_name")
